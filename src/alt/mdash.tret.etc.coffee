...
class Mdash.Tret.Etc extends Mdash.Tret
  ...
  rules:
    ...
    word_sup:
      ...
      replacement: (match, m) -> Mdash.Lib.tag(Mdash.Lib.tag(m[3], "small"), "sup") + m[4]
    century_period:
      description: 'Тире между диапозоном веков'
      pattern: /(\040|\t|\&nbsp\;|^)([XIV]{1,5})(-|\&mdash\;)([XIV]{1,5})(( |\&nbsp\;)?(в\.в\.|вв\.|вв|в\.|в))/g
      replacement: (match, m) -> m[1]  + Mdash.Lib.tag("#{m[2]}&mdash;#{m[4]} вв.", "span", {class: "nowrap"})
    time_interval:
      description: 'Тире и отмена переноса между диапозоном времени'
      pattern: /([^\d\>]|^)([\d]{1,2}\:[\d]{2})(-|\&mdash\;|\&minus\;)([\d]{1,2}\:[\d]{2})([^\d\<]|$)/ig
      replacement: (match, m) -> m[1] + Mdash.Lib.tag("#{m[2]}&mdash;#{m[4]}", "span", {class: "nowrap"}) + m[5]
  ...
  remove_nbsp: (text) ->
    thetag = Mdash.Lib.tag("###", 'span', {class: "nowrap"})
    arr = thetag.split("###")
