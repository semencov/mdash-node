class Mdash.Tret.Date extends Mdash.Tret
  ...
  rules:
    ...
    years:
      ...
      replacement: -> "#{$1}#{$2}" + (if parseInt($3) >= parseInt($5) then "#{$3}#{$4}#{$5}" else "#{$3}&mdash;#{$5}") + (if $6? then "&nbsp;гг" else "") + (if $9? then $9 else "")
    ...
    nbsp_and_dash_month_interval:
      ...
      replacement: -> $1 + @tag("#{$2}&mdash;#{$4} #{$6}", "span", {class: "nowrap"}) + $7
      nowrap: true
