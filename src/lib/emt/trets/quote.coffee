...
class EMTretQuote extends EMTret
  ...
  inject_in: (index, string) ->
    @_text = @_text.substring(0, index) + string + @_text.substring(index + string.length, @_text.length)
  ...
  build_sub_quotations: () ->
    ...
    while(true)
      p = EMTLib.strpos_ex @_text, ["&laquo;", "&raquo;"], offset
      break  if p is false
        ...
        if level < 0
          if not @is_on('no_inches')
            ...
            while amount is 0 and okposstack.length
              ...
              k = @_text.substr lokpos, offset-lokpos
          ...
          if amount is 1
            @_text = @_text.substr(0, lokpos) + k + @_text.substr(offset)
            offset = lokpos
            level = 0
            continue
          ...
          if amount is 0
            level = 0
            @_text = @_text.substr(0, p.pos) + '&quot;' + @_text.substr(offset)
            offset = p.pos + "&quot;".length
            okposstack = [offset]
            continue
    ...
    if level isnt 0
      if level > 0
        k = @_text.substr okpos
        k = k.replace @QUOTE_CRAWSE_OPEN, @QUOTE_FIRS_OPEN
        k = k.replace @QUOTE_CRAWSE_CLOSE, @QUOTE_FIRS_CLOSE
        @_text = @_text.substr(0, okpos) + k
