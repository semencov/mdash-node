...
class EMTret
  ...
  log: (str, data=null) ->
    ...
    @logs.push {info: str, data: data}
    return

  error: (info, data=null) ->
    @errors.push {info: info, data: data}
    @log "ERROR: #{info}", data
    return
  
  debug: (place, after_text) ->
    return  if not @debug_enabled
    @debug_info.push {place: place, text: after_text}
    return
   ...
   * @param int $kind
  ###
  set_tag_layout: (layout=@LAYOUT_STYLE) ->
    @use_layout = layout
    @use_layout_set = true
  ...
  apply_rule: (rule) ->
    name = rule.id
    disabled = (@disabled[rule.id] or rule.disabled) and !@enabled[rule.id]

    if disabled
      @log "Правило #{name}", "Правило отключено" + (if rule.disabled then " (по умолчанию)" else "")
      return
    ...
    if rule.function?
      ...
      if typeof fn is 'function'
        @log "Правило #{name}", "Используется метод #{rule.function} в правиле"
        @_text = fn.call(@, @_text)
        return

      @error "Функция #{rule.function} из правила #{rule.id} не найдена"
      return
    else if rule.pattern?
      patterns = rule.pattern
      patterns = new RegExp(EMTLib.preg_quote patterns, 'g')  if typeof patterns is 'string'
      patterns = [ patterns ]  if !Array.isArray patterns

      for k, pattern of patterns
        pattern = new RegExp(EMTLib.preg_quote pattern, 'g')  if typeof pattern is 'string'
        ...
        if typeof replacement is 'string' and /^[a-z_0-9]+$/i.test replacement
          @log "Правило #{name}", "Замена с использованием replace с методом #{replacement}"
          replacement = @[replacement]  if {}.hasOwnProperty.call(@, replacement)
          replacement = eval(replacement)  if eval("typeof " + replacement)

        self = @
        @_text = @_text.replace pattern, if typeof replacement is 'string' then replacement else () ->
  ...
  _apply: (list) ->
    ...
    @log "Применяется набор правил", list.join(",")
  ...
  tag: (content, tag='span', attribute={}) ->
    ...
    layout = if @use_layout is false then @LAYOUT_STYLE else @use_layout
    ...
    if layout is @LAYOUT_CLASS and classname
      htmlTag += " class=\"#{classname}\""
    
    "<" + EMTLib.encrypt_tag(htmlTag) + ">#{content}</" + EMTLib.encrypt_tag(tag) + ">"
