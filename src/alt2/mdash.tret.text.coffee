...
class Mdash.Tret.Text extends Mdash.Tret
  ...
  rules:
    ...
    auto_links:
      ...
      replacement: [
        -> $1 + Mdash.Lib.tag((if $4.substr(id.length-1) is "." then $4.substr(0, id.length-1) else $4), "a", {href: "#{$2}#{$3}" + (if $4.substr(id.length-1) is "." then $4.substr(0, id.length-1) else $4)}) + (if $4.substr(id.length-1) then "." else "") + $5
      ]
    ...
    email:
      ...
      replacement: [
        -> $1 + Mdash.Lib.tag("#{$2}@#{$3}.#{$4}", "a", {href: "mailto:#{$2}@#{$3}.#{$4}"}) + $5
      ]
    ...
    breakline:
      function: (text) ->
        text = text.replace new RegExp("(\<\/#{@BASE64_PARAGRAPH_TAG}\>)([\r\n \t]+)(\<#{@BASE64_PARAGRAPH_TAG}\>)", 'g'), ($0, $1, $2, $3) ->
          "#{$1}#{Mdash.Lib.INTERNAL_BLOCK_OPEN}" + Mdash.Lib.encode($2) + "#{Mdash.Lib.INTERNAL_BLOCK_CLOSE}#{$3}"
  ...
  do_paragraphs: (text) ->
    ...
    text = text.replace /([\s\t]+)?(\n)+([\s\t]*)(\n)+/g, ($0, $1="", $2, $3) ->
      "#{$1}<#{self.BASE64_PARAGRAPH_TAG}>#{Mdash.Lib.INTERNAL_BLOCK_OPEN}" + Mdash.Lib.encode("#{$2}#{$3}") + "#{Mdash.Lib.INTERNAL_BLOCK_CLOSE}</#{self.BASE64_PARAGRAPH_TAG}>"
